        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('avatar', function ($data) {
                    return '<img src="' . asset($data->avatar) . '" alt="Avatar" class="rounded-circle" width="50" height="50">';
                })
                ...
                ->addColumn('action', function ($data) {
                    $editButton = '';
                    if (Auth::user()->is_admin && $data->getRoleNames()->first() == 'admin') {
                ...
                ->rawColumns(['bulk_check','avatar', 'role', 'status', 'action'])
                ->make(true);
        }
        $data['roles'] = Role::all();

        return view('backend.layout.user.index', $data);
    }
    ...
    public function update(Request $request)
    {
        try {
            ...
            if ($request->hasFile('avatar')) {
                if ($user->avatar && file_exists($user->avatar) && $user->avatar != 'user.png') {
                    unlink($user->avatar);
                }
